...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    //设置边框颜色
    //layer：每一个视图都拥有一个layer.layer一般做一些高级操作，图层渲染这些。
    ...
    //按钮高亮状态，名称为“高亮状态”，地按住按钮不放开，就是高亮状态；
...
//    NSLog(@"imagePath==%@",imagePath);
   ...
   //获取图片的宽高
    ...
    //setImage 图片不会变形
    //setBackgroundImage     图片根据button的frame改变而改变；
    //给按钮添加事件。      就是用户点击按钮的时候，干了一件事情
    //addTarget          给谁添加一个事件，self代表给本类添加一个事件
    //action             时间内容
    //forControlEvents   如何触发这个事件。UIControlEventTouchUpInside带表按钮按下并抬起事件
    //如果添加了事件，但是又没实现这个Selector，会奔溃；
...
-(void)buttonClick:(UIButton*)button
{
    ...
    UIButton*buttin2=(UIButton*)[self.window viewWithTag:102];

    NSLog(@"button1==%@",button1.currentTitle);
    NSLog(@"button2==%@",buttin2.currentTitle);
}
